...
/*
Copyright (C) 2005 Peter N. Robinson, M.D.
...
/**
 * Ramachandran.java
 * This program takes a file from the command line that
 * contains sets of psi,phi angles and plots the
 * corresponding Ramachandran plot.
 * Format of the input file:
 * psi\tphi\n
 * psi\tphi\n ...etc
 * The graphic is displayed on the screen and written to a file
 *  'ramachandran.png'
 * @author Peter N. Robinson
 * @version 0.31, July 16, 2005
 */
public class Ramachandran extends JPanel {
	...
	int selection = JOptionPane.showConfirmDialog(null,
	     "Do you want to save this plot as \"ramachandran.png\"?" ,
						      "Sample",
	...
	int i = 0;
		...
		String curArg, curValue;
		if (current.startsWith("-")) {
		    switch (current.charAt(1)) {
		    case 'k' :
			kleywegt = true;
			break;
		    case 'f':  // The filename + path
			if (argv[i+1] != null){
			    inputData(argv[i+1]);
			    needdata = false;
			} else {
			...
			if (argv[i+1] != null){
			    String box = argv[i+1];
			    String[] stArray = box.split(":");
			    if (stArray.length != 2) {
				usage();
			    }
			    int x,y;
			    x = Integer.parseInt(stArray[0]);
			    y = Integer.parseInt(stArray[1]);
			    if (java.lang.Math.abs(x) >= 150 || 
				java.lang.Math.abs(y) >= 150) {
				usage();
			    }
			    this.boxX = x;
			    this.boxY = y;
			...
			System.err.println("Error: Argument needs to "
					   + " start with \"-\"");
			usage();
		    }
		    ++i;
	...
	if (needdata) usage();  // no data file entered.
    }
	...
	try {
	    ...
	    System.err.println("Read " + count + " lines of data");
	} catch (Exception e) {
